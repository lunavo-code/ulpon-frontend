<template>
    ...
    >
        ...
        <div v-if="templateOptions.length > 0 && displayColumns.length > 1" class="template-select-header">
      <span
      ...
      >
        ...
        <el-option
            ...
            :key="item.id"
            :label="String(item[displayColumns[0] as keyof TemplateVO] ?? '')"
            :value="item[valueKey as keyof TemplateVO]"
        >
            <div class="template-select-option-row">
        <span
            v-for="(col, index) in displayColumns"
            :key="col"
            class="option-cell"
...
<script setup name="TemplateSelect" lang="ts">
    import { PropType } from 'vue';
    import { useDebounceFn } from '@vueuse/core';
    import { listTemplate } from '@/api/tool/gen/template';
    import { TemplateVO } from '@/api/tool/gen/template/types';

    // 生成实例级唯一的 popper 样式类，防止多个下拉框事件冲突
    const uniqueClass = `template-select-popper-${Math.random().toString(36).substring(2, 9)}`;
    ...
    // 运行中下拉框里展示的列表数据
    const templateOptions = ref<TemplateVO[]>([]);
    ...
    const defaultOptions = ref<TemplateVO[]>([]);
    ...
    const columnLabelsMap: Record<string, string> = {
            id: 'id',
            version: '版本',
            type: '模板类型: backend,frontend-vue,frontend-react,sql',
            path: '生成路径',
            name: '模板名称',
            content: '模板内容',
            sort: '排序',
    ...
    const hasMore = computed(() => templateOptions.value.length < total.value);
        ...
        if (!isSearch) {
            ...
            if (!isAppend) {
                ...
                if (defaultOptions.value.length > 0) {
                    templateOptions.value = [...defaultOptions.value];
            ...
            try {
                ...
                const rows = res.data?.rows || [];
                if (isAppend) {
                    templateOptions.value.push(...rows);
                } else {
    ...
    const handleScroll = (e: Event) => {
        ...
        if (scrolledItems >= templateOptions.value.length - thresholdOffset) {
            loadMore();
        }
    };
