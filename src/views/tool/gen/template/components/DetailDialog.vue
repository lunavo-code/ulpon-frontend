<template>
  <el-dialog v-model="dialog.visible" :title="dialog.title" width="800px" append-to-body>
    <el-form class="dialog-grid-form" ref="templateFormRef" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="模板组ID" prop="templateSetId">
        <el-input-number v-model="form.templateSetId" controls-position="right"/>
      </el-form-item>
      <el-form-item label="模板标识" prop="code">
        <el-input v-model="form.code" placeholder="请输入模板标识"/>
      </el-form-item>
      <el-form-item label="模板名称" prop="name">
        <el-input v-model="form.name" placeholder="请输入模板名称"/>
      </el-form-item>
      <el-form-item label="模板类型" prop="type">
        <enum-radio v-model="form.type" model="gen" enum-type="TemplateCategoryEnum" placeholder="请选择模板类型" clearable/>
      </el-form-item>
      <el-form-item label="模板版本" prop="version">
        <el-input v-model="form.version" placeholder="请输入模板版本"/>
      </el-form-item>
      <el-form-item label="输出目录模板" prop="path">
        <el-input v-model="form.path" type="textarea" placeholder="请输入内容"/>
      </el-form-item>
      <el-form-item label="输出文件名模板" prop="fileName">
      ...
      <el-form-item label="状态：1启用 0禁用" prop="status">
        <dict-radio v-model="form.status" dict-type="sys_normal_disable" placeholder="请选择字典生成或手动指定元素列表"/>
      </el-form-item>
    </el-form>
    <template #footer>
...
import {getTemplate, addTemplate, updateTemplate} from '@/api/tool/gen/template';
...
const data = reactive<PageData<TemplateForm, any>>({
  ...
  queryParams: {},
  rules: {
    id: [{ required: true, message: '模板ID不能为空', trigger: "change" }],
    ...
    version: [{ required: true, message: '模板版本不能为空', trigger: "blur" }],
    path: [{ required: true, message: '输出目录模板不能为空', trigger: "blur" }],
    fileName: [{ required: true, message: '输出文件名模板不能为空', trigger: "blur" }],
    content: [{ required: true, message: '模板内容不能为空', trigger: "blur" }],
    status: [{ required: true, message: '状态：1启用 0禁用不能为空', trigger: "change" }],
  }
});
