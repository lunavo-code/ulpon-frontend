<template>
    ...
    >
        ...
        <div v-if="agentOptions.length > 0 && displayColumns.length > 1" class="agent-select-header">
      <span
      ...
      >
        ...
        <el-option
            ...
            :key="item.agentId"
            :label="String(item[displayColumns[0] as keyof AgentVO] ?? '')"
            :value="item[valueKey as keyof AgentVO]"
        >
            <div class="agent-select-option-row">
        <span
            v-for="(col, index) in displayColumns"
            :key="col"
            class="option-cell"
...
<script setup name="AgentSelect" lang="ts">
    import { PropType } from 'vue';
    import { useDebounceFn } from '@vueuse/core';
    import { listAgent } from '@/api/ai/agent';
    import { AgentVO } from '@/api/ai/agent/types';

    // 生成实例级唯一的 popper 样式类，防止多个下拉框事件冲突
    const uniqueClass = `agent-select-popper-${Math.random().toString(36).substring(2, 9)}`;
    ...
    const defaultOptions = ref<AgentVO[]>([]);
    ...
    const columnLabelsMap: Record<string, string> = {
            ...
            kbEnabled: '启用知识库（0停用 1启用）',
            kbId: '关联知识库 ID',
            searchEnabled: '启用联网检索（0停用 1启用）',
            memoryEnabled: '启用聊天记忆（0停用 1启用）',
            memoryWindow: '记忆窗口长度',
            greeting: '问候语',
    ...
    const hasMore = computed(() => agentOptions.value.length < total.value);
        ...
        if (!isSearch) {
            ...
            if (!isAppend) {
                ...
                if (defaultOptions.value.length > 0) {
                    agentOptions.value = [...defaultOptions.value];
    ...
    const handleScroll = (e: Event) => {
        ...
        if (scrolledItems >= agentOptions.value.length - thresholdOffset) {
            loadMore();
        }
    };
