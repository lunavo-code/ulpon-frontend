<template>
    ...
    >
        ...
        <div v-if="chatSessionOptions.length > 0 && displayColumns.length > 1" class="chatSession-select-header">
      <span
      ...
      >
        ...
        <!-- 数据选项列表 -->
        <el-option
            v-for="item in chatSessionOptions"
            :key="item.sessionId"
            :label="String(item[displayColumns[0] as keyof ChatSessionVO] ?? '')"
            :value="item[valueKey as keyof ChatSessionVO]"
        >
            <div class="chatSession-select-option-row">
        <span
            v-for="(col, index) in displayColumns"
            :key="col"
            class="option-cell"
            :style="{ flex: 1, marginRight: index < displayColumns.length - 1 ? '16px' : '0' }"
        >
          {{ item[col as keyof ChatSessionVO] }}
        </span>
            </div>
        </el-option>
...
<script setup name="ChatSessionSelect" lang="ts">
    import { PropType } from 'vue';
    import { useDebounceFn } from '@vueuse/core';
    import { listChatSession } from '@/api/ai/chatSession';
    import { ChatSessionVO } from '@/api/ai/chatSession/types';

    // 生成实例级唯一的 popper 样式类，防止多个下拉框事件冲突
    const uniqueClass = `chatSession-select-popper-${Math.random().toString(36).substring(2, 9)}`;
    ...
    // 运行中下拉框里展示的列表数据
    const chatSessionOptions = ref<ChatSessionVO[]>([]);
    ...
    const defaultOptions = ref<ChatSessionVO[]>([]);
    ...
    const hasMore = computed(() => chatSessionOptions.value.length < total.value);
        ...
        if (!isSearch) {
            ...
            if (!isAppend) {
                ...
                if (defaultOptions.value.length > 0) {
                    chatSessionOptions.value = [...defaultOptions.value];
                    pageNum.value = defaultPageNum.value;
                    total.value = defaultTotal.value;
                    return;
                }
                pageNum.value = 1;
                chatSessionOptions.value = [];
                loading.value = true;
            } else {
            ...
            try {
                ...
                const rows = res.data?.rows || [];
                if (isAppend) {
                    chatSessionOptions.value.push(...rows);
                } else {
                    chatSessionOptions.value = rows;
                }
    ...
    const handleScroll = (e: Event) => {
        ...
        if (scrolledItems >= chatSessionOptions.value.length - thresholdOffset) {
            loadMore();
        }
    };
    ...
    const handleVisibleChange = (visible: boolean) => {
        if (visible) {
            ...
            if (chatSessionOptions.value.length === 0) {
                loadOptions();
            }
            bindScrollListener();
        } else {
