<template>
    ...
    >
        ...
        <div v-if="modelConfigOptions.length > 0 && displayColumns.length > 1" class="modelConfig-select-header">
      <span
      ...
      >
        ...
        <el-option
            ...
            :label="String(item[displayColumns[0] as keyof ModelConfigVO] ?? '')"
            :value="item[valueKey as keyof ModelConfigVO]"
        >
            <div class="modelConfig-select-option-row">
        <span
            v-for="(col, index) in displayColumns"
            :key="col"
            class="option-cell"
            :style="{ flex: 1, marginRight: index < displayColumns.length - 1 ? '16px' : '0' }"
        >
          {{ item[col as keyof ModelConfigVO] }}
        </span>
            </div>
        </el-option>
...
<script setup name="ModelConfigSelect" lang="ts">
    import { PropType } from 'vue';
    import { useDebounceFn } from '@vueuse/core';
    import { listModelConfig } from '@/api/ai/modelConfig';
    import { ModelConfigVO } from '@/api/ai/modelConfig/types';

    // 生成实例级唯一的 popper 样式类，防止多个下拉框事件冲突
    const uniqueClass = `modelConfig-select-popper-${Math.random().toString(36).substring(2, 9)}`;
    ...
    // 运行中下拉框里展示的列表数据
    const modelConfigOptions = ref<ModelConfigVO[]>([]);
    ...
    const defaultOptions = ref<ModelConfigVO[]>([]);
    ...
    const columnLabelsMap: Record<string, string> = {
            ...
            modelName: '目标模型名称',
            baseUrl: 'API 接口覆盖地址',
            apiKey: 'API 密钥 (Script Key)',
            maxTokens: '最大生成 Token 数',
            temperature: '温度参数 (0.0 ~ 2.0)',
            status: '启用状态（0正常 1停用）',
            isDefault: '是否为默认模型（Y是 N否）',
    ...
    const hasMore = computed(() => modelConfigOptions.value.length < total.value);
        ...
        if (!isSearch) {
            ...
            if (!isAppend) {
                ...
                if (defaultOptions.value.length > 0) {
                    modelConfigOptions.value = [...defaultOptions.value];
            ...
            try {
                const res = await listModelConfig({
            ...
            try {
                ...
                const rows = res.data?.rows || [];
                if (isAppend) {
                    modelConfigOptions.value.push(...rows);
                } else {
                    modelConfigOptions.value = rows;
                }
    ...
    const handleScroll = (e: Event) => {
        ...
        if (scrolledItems >= modelConfigOptions.value.length - thresholdOffset) {
            loadMore();
        }
    };
    ...
    const handleVisibleChange = (visible: boolean) => {
        if (visible) {
            ...
            if (modelConfigOptions.value.length === 0) {
                loadOptions();
            }
            bindScrollListener();
        } else {
