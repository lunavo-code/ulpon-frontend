    ...
    <el-card shadow="hover" class="table-panel">
        <template #header>
            <div class="toolbar-shell">
                <div class="table-heading"><h3>对话消息列表</h3></div>
                <div class="toolbar-actions">
                    <el-button v-hasPermi="['ai:chatMessage:add']" type="primary" plain icon="Plus" @click="handleAdd">
                        新增
                    </el-button>
                    <el-button v-hasPermi="['ai:chatMessage:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()">
                        修改
                    </el-button>
                    <el-button v-hasPermi="['ai:chatMessage:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()">
                        删除
                    </el-button>
                    <el-button v-hasPermi="['ai:chatMessage:export']" type="warning" plain icon="Download" @click="handleExport">
                        导出
                    </el-button>
        ...
        <el-table v-loading="loading" border class="data-table" :data="chatMessageList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template #default="scope">
                    <el-tooltip content="修改" placement="top">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['ai:chatMessage:edit']"></el-button>
                    </el-tooltip>
                    <el-tooltip content="删除" placement="top">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['ai:chatMessage:remove']"></el-button>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
...
<script setup name="ChatMessageTablePanel" lang="ts">
    import {listChatMessage, delChatMessage} from '@/api/ai/chatMessage';
    import {ChatMessageVO, ChatMessageQuery} from '@/api/ai/chatMessage/types';
    ...
    const queryParams = reactive < ChatMessageQuery > ({
        pageNum: 1,
        pageSize: 10,
        messageId: undefined,
        sessionId: undefined,
    ...
    const {ids, single, multiple, handleSelectionChange} = useTableSelection < ChatMessageVO > (item => item.messageId);

    /** 查询对话消息列表 */
    const getList = async () => {
        await withLoading(async () => {
            let params = queryParams.value;
            const res = await listChatMessage(params);
            chatMessageList.value = res.data?.rows || [];
            total.value = res.data?.total || 0;
        });
    };

    /** 外部应用查询过滤条件 */
    const applyFilters = (filters: Partial<ChatMessageQuery>) => {
    ...
    const handleUpdate = (row?: any) => {
        const id = row?.messageId || ids.value[0];
        emit('edit', id);
    };

    /** 删除按钮操作 */
    const handleDelete = async (row?: any) => {
        const targetIds = row?.messageId ? [row.messageId] : ids.value;
        await modal.confirm('是否确认删除测试单编号为"' + targetIds + '"的数据项？');
        await withLoading(async () => {
            await delChatMessage(targetIds);
        });
    ...
    const handleExport = () => {
        requestDownload(
            'chatMessage/chatMessage/export',
            {
                ...queryParams
            },
            `对话消息_ChatMessage_${new Date().getTime()}.xlsx`
        );
    };
