<template>
    ...
    >
        ...
        <div v-if="chatMessageOptions.length > 0 && displayColumns.length > 1" class="chatMessage-select-header">
      <span
      ...
      >
        ...
        <!-- 数据选项列表 -->
        <el-option
            v-for="item in chatMessageOptions"
            :key="item.messageId"
            :label="String(item[displayColumns[0] as keyof ChatMessageVO] ?? '')"
            :value="item[valueKey as keyof ChatMessageVO]"
        >
            <div class="chatMessage-select-option-row">
        <span
            v-for="(col, index) in displayColumns"
            :key="col"
            class="option-cell"
            :style="{ flex: 1, marginRight: index < displayColumns.length - 1 ? '16px' : '0' }"
        >
          {{ item[col as keyof ChatMessageVO] }}
        </span>
            </div>
        </el-option>
...
<script setup name="ChatMessageSelect" lang="ts">
    import { PropType } from 'vue';
    import { useDebounceFn } from '@vueuse/core';
    import { listChatMessage } from '@/api/ai/chatMessage';
    import { ChatMessageVO } from '@/api/ai/chatMessage/types';

    // 生成实例级唯一的 popper 样式类，防止多个下拉框事件冲突
    const uniqueClass = `chatMessage-select-popper-${Math.random().toString(36).substring(2, 9)}`;
    ...
    // 运行中下拉框里展示的列表数据
    const chatMessageOptions = ref<ChatMessageVO[]>([]);
    ...
    const defaultOptions = ref<ChatMessageVO[]>([]);
    ...
    const hasMore = computed(() => chatMessageOptions.value.length < total.value);
        ...
        if (!isSearch) {
            ...
            if (!isAppend) {
                ...
                if (defaultOptions.value.length > 0) {
                    chatMessageOptions.value = [...defaultOptions.value];
                    pageNum.value = defaultPageNum.value;
                    total.value = defaultTotal.value;
                    return;
                }
                pageNum.value = 1;
                chatMessageOptions.value = [];
                loading.value = true;
            } else {
            ...
            try {
                const res = await listChatMessage({
            ...
            try {
                ...
                const rows = res.data?.rows || [];
                if (isAppend) {
                    chatMessageOptions.value.push(...rows);
                } else {
                    chatMessageOptions.value = rows;
                }
    ...
    const handleScroll = (e: Event) => {
        ...
        if (scrolledItems >= chatMessageOptions.value.length - thresholdOffset) {
            loadMore();
        }
    };
    ...
    const handleVisibleChange = (visible: boolean) => {
        if (visible) {
            ...
            if (chatMessageOptions.value.length === 0) {
                loadOptions();
            }
            bindScrollListener();
        } else {
