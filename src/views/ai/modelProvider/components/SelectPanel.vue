<template>
  ...
  >
    ...
    <div v-if="modelProviderOptions.length > 0 && displayColumns.length > 1" class="modelProvider-select-header">
      <span
    ...
    <el-option
      ...
      :key="item.providerId"
      :label="String(item[displayColumns[0] as keyof ModelProviderVO] ?? '')"
      :value="item[valueKey as keyof ModelProviderVO]"
    >
      <div class="modelProvider-select-option-row">
        <span
          v-for="(col, index) in displayColumns"
          :key="col"
          class="option-cell"
          :style="{ flex: 1, marginRight: index < displayColumns.length - 1 ? '16px' : '0' }"
        >
          {{ item[col as keyof ModelProviderVO] }}
        </span>
      </div>
    </el-option>
...
<script setup name="ModelProviderSelect" lang="ts">
import {PropType} from 'vue';
import {useDebounceFn} from '@vueuse/core';
import {listModelProvider} from '@/api/ai/modelProvider';
import {ModelProviderVO} from '@/api/ai/modelProvider/types';

// 生成实例级唯一的 popper 样式类，防止多个下拉框事件冲突
const uniqueClass = `modelProvider-select-popper-${Math.random().toString(36).substring(2, 9)}`;
...
// 运行中下拉框里展示的列表数据
const modelProviderOptions = ref<ModelProviderVO[]>([]);
...
const defaultOptions = ref<ModelProviderVO[]>([]);
...
const columnLabelsMap: Record<string, string> = {
  providerId: '主键 ID',
  providerName: '提供商名称',
  providerCode: '提供商唯一标识键',
  defaultBaseUrl: '默认 API 接口地址',
  status: '启用状态（0正常 1停用）',
...
const hasMore = computed(() => modelProviderOptions.value.length < total.value);
  ...
  if (!isSearch) {
    ...
    if (!isAppend) {
      ...
      if (defaultOptions.value.length > 0) {
        modelProviderOptions.value = [...defaultOptions.value];
    ...
    try {
      const res = await listModelProvider({
    ...
    try {
      ...
      const rows = res.data?.rows || [];
      if (isAppend) {
        modelProviderOptions.value.push(...rows);
      } else {
        modelProviderOptions.value = rows;
      }
...
const handleScroll = (e: Event) => {
  ...
  if (scrolledItems >= modelProviderOptions.value.length - thresholdOffset) {
    loadMore();
  }
};
...
const handleVisibleChange = (visible: boolean) => {
  if (visible) {
    ...
    if (modelProviderOptions.value.length === 0) {
      loadOptions();
    }
    bindScrollListener();
  } else {
...
/* 选项行样式 */
.modelProvider-select-option-row {
  display: flex;
  justify-content: space-between;
  align-items: center;
  width: 100%;
