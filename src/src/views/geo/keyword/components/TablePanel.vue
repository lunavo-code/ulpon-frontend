    ...
    <el-card shadow="hover" class="table-panel">
        <template #header>
            <div class="toolbar-shell">
                <div class="table-heading"><h3>核心词管理列表</h3></div>
                <div class="toolbar-actions">
                    <el-button v-hasPermi="['geo:keyword:add']" type="primary" plain icon="Plus" @click="handleAdd">
                        新增
                    </el-button>
                    <el-button v-hasPermi="['geo:keyword:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()">
                        修改
                    </el-button>
                    <el-button v-hasPermi="['geo:keyword:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()">
                        删除
                    </el-button>
                    <el-button v-hasPermi="['geo:keyword:export']" type="warning" plain icon="Download" @click="handleExport">
                        导出
                    </el-button>
        ...
        <el-table v-loading="loading" border class="data-table" :data="keywordList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template #default="scope">
                    <el-tooltip content="修改" placement="top">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['geo:keyword:edit']"></el-button>
                    </el-tooltip>
                    <el-tooltip content="删除" placement="top">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['geo:keyword:remove']"></el-button>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
...
<script setup name="KeywordTablePanel" lang="ts">
    import {listKeyword, delKeyword} from '@/api/geo/keyword';
    import {KeywordVO, KeywordQuery} from '@/api/geo/keyword/types';
    ...
    const {ids, single, multiple, handleSelectionChange} = useTableSelection < KeywordVO > (item => item.keywordId);

    /** 查询核心词管理列表 */
    const getList = async () => {
        await withLoading(async () => {
            let params = queryParams.value;
            const res = await listKeyword(params);
            keywordList.value = res.data?.rows || [];
            total.value = res.data?.total || 0;
        });
    };

    /** 外部应用查询过滤条件 */
    const applyFilters = (filters: Partial<KeywordQuery>) => {
        queryParams.pageNum = 1;
        // 清理先前的过滤参数，防止属性残留污染
        queryParams.companyName = undefined;
        queryParams.keywordType = undefined;
    ...
    const handleUpdate = (row?: any) => {
        const id = row?.keywordId || ids.value[0];
        emit('edit', id);
    };

    /** 删除按钮操作 */
    const handleDelete = async (row?: any) => {
        const targetIds = row?.keywordId ? [row.keywordId] : ids.value;
        await modal.confirm('是否确认删除测试单编号为"' + targetIds + '"的数据项？');
        await withLoading(async () => {
            await delKeyword(targetIds);
        });
    ...
    const handleExport = () => {
        requestDownload(
            ...
            `核心词管理_Keyword_${new Date().getTime()}.xlsx`
        );
    };
